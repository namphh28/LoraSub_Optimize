...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10):
        ...
        self.initlora_A_k = torch.zeros(dim, r)
        self.initlora_A_v = torch.zeros(dim, r)
    ...
    def init_param(self):
        for t in range(len(self.lora_A_k)):
            ...
            kr = 1.0 / self.dim
            nn.init.sparse_(self.lora_B_k[t].weight, sparsity=(1 - kr))
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_cur_x=False):
        B, N, C = x.shape
        ...
        # x_c = x.detach()
        # qkv2 = self.qkv(x_c).reshape(B, N, 3, C).permute(2, 0, 1, 3)
